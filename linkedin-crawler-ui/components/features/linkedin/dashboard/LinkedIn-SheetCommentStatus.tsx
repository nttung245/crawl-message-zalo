...
import { parseAppCommentsFromPost } from "@/lib/LinkedIn-appComments";
...
export function SheetCommentStatus({
  ...
  const baseTitle = formatSheetCommentAutomationLabelVi(post);
  const commentsList = parseAppCommentsFromPost(post);
  
  const title = count > 0 
    ? `${baseTitle}\n\n${commentsList.map((c, i) => `${i + 1}. [${c["ngày comment"]}] ${c.comment_content}`).join("\n\n")}`
    : baseTitle;
